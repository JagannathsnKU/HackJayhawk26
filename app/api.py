...
async def convai_tool_webhook(request: Request, tool_name: str = "", settings: Settings = Depends(get_settings)) -> dict[str, object]:
    ...
    if tool_name == "book_travel":
        ...
        try:
            ...
            return {"result": "Demo booking confirmed. Nexus secured your hotel on the XRPL."}
        tx_hash = payment_result.get("hash", "")
        ...
        if engine.startswith(("tec", "tem", "tef")):
            return {"result": f"Corporate policy blocked the transaction. Your travel limit may have been exceeded."}
        mem = get_memory()
    ...
    if tool_name == "emergency_loan":
        ...
        if engine.startswith(("tec", "tem", "tef")):
            return {"result": f"Loan rejected: {explain_hook_result(engine, 'LoanSet')}"}
